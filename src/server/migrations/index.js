...
function buildMigrationPath(data) {
	...
	let maxMigration = data.migrations.length ? data.migrations[0] : null;
	...
	if (!maxMigration || compareVersion(maxMigration.version, data.version) === 0) {
		logger.info('Database version %s is actual.', data.version);
		return [];
	}

	if (compareVersion(maxMigration.version, data.version) === -1) {
		logger.error('Database version is higher than latest migration (%s).', data.version);
		return [];
	}
...
function migrateToActual() {
	...
	return Promise.all([version, migrations])
		...
		.then((path) => {
			return Promise.mapSeries(path, (item) => {
...
// Module exports
module.exports = {
	migrateToActual: migrateToActual
};
